...
if (!defined('_DESIGNER_FUNCTIONS')) {
    ...
    function getProductsScript() {
        ...
        return '<script type="application/javascript"> window._npIsCms = true; window._npProductsJsonUrl = "' . $url . '";</script>';
    }

    function getThankYouScript() {
        $url = Uri::root() . 'index.php/component/thankyou/?tmpl=thankyou';
        return '<script type="application/javascript"> window._npThankYouUrl = "' . $url . '";</script>';
    }
    ...
    function checkAndGetLanguage()
    {
        ...
        $defaultThemeLang = 'de';
        $themeLangs = json_decode('[{"id":"1","name":"de","title":"German"}]', true);
...
function block_sidebar_custom_products_1($caption, $content) {
    ...
    function getSvgSizes($path) {
        ...
        if (!isset($svg['width']) || !isset($svg['height'])) {
            ...
            if ($viewBox) {
                // Формат viewBox: "min-x min-y width height"
                $viewBoxParts = explode(' ', $viewBox);
                if (count($viewBoxParts) == 4) {
                    $sizes['width'] = $viewBoxParts[2];
                    $sizes['height'] = $viewBoxParts[3];
                }
            }
        } else {
            $sizes['width'] = (string)$svg['width'];
            $sizes['height'] = (string)$svg['height'];
        }
    ...
    function getLogoInfo($defaults = array(), $setThemeSizes = false)
    {
        ...
        if ($isSvgFile) {
            $logoSizes = getSvgSizes($info['src_path']);
            $info['width'] = $logoSizes['width'];
            $info['height'] = $logoSizes['height'];
        } else {
            $imgSize = getimagesize($info['src_path']);
            $info['width'] = $imgSize ? $imgSize[0] : '';
            $info['height'] = $imgSize ? $imgSize[1] : '';
        }
    ...
    function getCategoriesFilterOptions($categories, $optionTemplate, $isJoomlaData, $level = 0)
    {
        $result = '';
        foreach ($categories as $category) {
            $value = $isJoomlaData ? $category->link : $category->id;
            ...
            if (count($category->children) > 0) {
                $result .= getCategoriesFilterOptions($category->children, $optionTemplate, $isJoomlaData, $level + 1);
            }
        }
        return $result;
    }
