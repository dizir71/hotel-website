...
switch($action) {
    ...
    case 'check_plg':
        ...
        break;
    case 'settings_is_set':
            require_once JPATH_ADMINISTRATOR . '/components/com_' . $name . '/helpers/nicepage.php';
            $config = NicepageHelpersNicepage::getConfig();
            if (isset($config['pluginSettings'])) {
                $response = 'ok';
            } else {
                $response = 'fail';
            }
        break;
    case 'apply_settings_plg':
        $value = $app->input->get('value', '0');
        $option = $app->input->get('pluginsettings', '');
        require_once JPATH_ADMINISTRATOR . '/components/com_' . $name . '/helpers/nicepage.php';
        $config = NicepageHelpersNicepage::getConfig();
        NicepageHelpersNicepage::saveConfig(array('pluginSettings' => $value));
        break;
    case 'import':
